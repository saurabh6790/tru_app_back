...
@webnotes.whitelist()
def make_provisional_sales_order(source_name, target_doclist=None):
	return _make_provisional_sales_order(source_name, target_doclist)
...
@webnotes.whitelist()
def set_as_negotiation_mode(source_name,target_doclist=None):
	webnotes.errprint(source_name)
	webnotes.conn.sql("""update `tabQuotation` set quotation_status='Negotiation Mode' where name='%s'"""%(source_name))
	webnotes.conn.sql('commit')
